...
if (nrow(art_results) > 0) {
  ...
  art_pal <- grDevices::colorRampPalette(c("#F6FAF6", "#D7E8D4", "#A7D0A0", "#6DB279", "#3E8D54", "#203B2B"))(120)

  open_png("00_art_factorial_main_metrics.png", width = 3300, height = 1750)
  ...
  for (r in seq_along(main_metric_order)) {
    for (c in seq_along(main_term_order)) {
      ...
      fill_col <- if (is.finite(score) && score > 0) {
        art_pal[max(1, min(length(art_pal), round(score / max_score * (length(art_pal) - 1)) + 1))]
      } else {
        "#F2F4F2"
      }
      ...
      text(c, yrow + 0.06, utf8_text(p_lab), cex = if (is_q_sig) 0.74 else 0.70,
           font = if (is_q_sig) 2 else 1, col = "#202423")
      text(c, yrow - 0.08, utf8_text(q_lab), cex = if (is_q_sig) 0.76 else 0.72,
           font = if (is_q_sig) 2 else 1, col = "#202423")
    }
  }

  axis(1, at = seq_along(main_term_order), labels = utf8_text(main_term_short), tick = FALSE, cex.axis = 1.04)
  axis(2, at = rev(seq_along(main_metric_order)), labels = utf8_text(unname(main_metric_label_map[main_metric_order])), las = 1, tick = FALSE, cex.axis = 1.00)
  box(col = "#D5DDD5", lwd = 1.1)
  mtext(utf8_text("Color = intensidad de evidencia (q, o p si q no está disponible); texto = p nominal y q FDR."), side = 1, outer = TRUE, line = 0.7, cex = 0.90, col = soft_ink)
  par(op)
  close_png()
}
